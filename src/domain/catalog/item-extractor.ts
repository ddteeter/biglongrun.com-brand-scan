...
- base_price_usd (number | null): the regular (non-sale) list price in USD for the default/lowest variant; null if price is not shown. When both a sale price and a regular/list price are displayed, extract the regular price — look for labels like "Was", "Reg.", "Regular", "MSRP", "List", strikethrough formatting, or a smaller-font alternative price. If only one price is visible, use that. The per-variant "price" field in per_size should follow the same rule: use the regular variant price, not the sale price.
